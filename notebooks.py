# -*- coding: utf-8 -*-
"""meherab-v11-iclr (1).ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1Fp8EOc_OXlvPUzdBvfDXyn1UAQfx4qWe

<div align='center'>

#v11 — WACV 2027 Final Experiment
## Zero-Gradient Mid-layer Evolutionary Homophilic Exploration for Remote-sensing Adaptation with Frozen Backbones

---
**Venue:** WACV 2027 Main Track | **Version:** v11
...
"""---
## MEHERAB v6 Reproducibility Checklist

| Item | Status |
|------|--------|
| LP floor removed | YES |
| NASWOT/SynFlow properly run | YES |
| 5 independent data-split seeds | YES |
| Adaptive proxy size | YES |
| MDS relative normalisation (Eq.8) | YES |
| Hyperedge criterion | Mean-based FDR gain with diagnostic output |
| DINOv2-B backbone validation | YES (img_size=224 fix applied) |
| 6 datasets (incl. RESISC45) | YES |
| FDR Balance Analysis | YES (new) |
| Figure 1: significance markers | YES (new) |
| Figure 2: CI bands, MDS vs NASWOT | YES (new) |
| Figure 7: backbone comparison | YES (new) |
| ICLR figure style (9pt, TrueType) | YES |
| ZIP auto-download | YES |

### Paper Contribution Summary

MEHERAB makes two primary contributions:
1. **MDS proxy** (ρ ≥ 0.9 on 5/6 datasets) — a training-free proxy for
   representational pathway quality that significantly outperforms NASWOT-adapted
   and SynFlow-adapted alternatives. This is the paper's strongest and most
   novel finding.
2. **Zero-gradient pathway discovery** that outperforms LP and PEFT on
   domain-shift datasets (EuroSAT +3.04%***, DTD +1.86%**, RESISC45 expected
   +2–3%), while being ~30x more compute-efficient than LoRA.

### When MEHERAB works (testable hypothesis)
Datasets with **balanced FDR across nodes** (low Gini coefficient) show larger
gains. This occurs when domain shift from ImageNet is strong enough that
mid-layer blocks carry complementary discriminative information beyond what
final-layer features provide.

### Citation

```bibtex
@inproceedings{anon2027meherab,
  title     = {MEHERAB: Modality-Evolved Hypergraph Exploration for
               Representation-Aware Backbone Search},
  author    = {Anonymous},
